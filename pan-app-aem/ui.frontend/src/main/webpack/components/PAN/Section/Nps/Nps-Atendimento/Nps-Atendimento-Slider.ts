export class NpsAtendimentoSlider {
  public $npsSlides: HTMLElement[];
  public $currentIndex: number;

  constructor() {
    this.$npsSlides = Array.from(
      document.querySelectorAll(`[data-js=nps-atendimento-slides]`)
    );
    this.$currentIndex = 0;
    setTimeout(() => {
  ...
  private setupEventListeners() {
    this.$npsSlides.forEach((slide, index) => {
      const nextButton = slide.querySelector(
        `[data-next=nps-atendimento-next]`
      );
      const prevButton = slide.querySelector(
        `[data-prev=nps-atendimento-prev]`
      );
